# Agent 365 Python Agent: Local Testing, MCP Tools, Observability, Notifications

## 1. Prerequisites

- Python 3.11+ installed
- [uv package manager](https://docs.astral.sh/uv/getting-started/installation/) (`pip install uv`)
- .NET 8.0+ (for some VS Code tooling)
- [A365 CLI](https://learn.microsoft.com/en-us/microsoft-agent-365/developer/agent-365-cli)
- [Agents Playground](https://learn.microsoft.com/en-us/microsoft-365/agents-sdk/test-with-toolkit-project) (`winget install agentsplayground` or `npm install -g @microsoft/m365agentsplayground`)

## 2. Environment Setup

- Copy `.env.template` to `.env`
- Fill in required values:
    - LLM keys (OpenAI or Azure OpenAI)
    - Agentic authentication (client ID, secret, tenant)
    - MCP endpoint (e.g., `http://localhost:5309` for local, or production endpoint)
    - Observability variables (see below)
...
## 4. Add/Configure MCP Tools

- Use the A365 CLI to add MCP servers:
    ```sh
    a365 develop add-permissions
    a365 develop get-token
    ```
- Edit `ToolingManifest.json` to include required MCP servers (Mail, Calendar, SharePoint, etc.)
- Reference: [Tooling servers overview](https://learn.microsoft.com/en-us/microsoft-agent-365/tooling-servers-overview)
...
- In `.env`, set:
    ...
    OBSERVABILITY_SERVICE_NAME=your-agent-service
    OBSERVABILITY_SERVICE_NAMESPACE=your-namespace
    ```
- Ensure your agent code calls the observability instrumentation (see `agent.py`):
    ```python
    from agent_framework.observability import AgentFrameworkInstrumentor
    AgentFrameworkInstrumentor().instrument()
    ```

## 6. Notification Handling

- Ensure your agent subscribes to notifications (see `agent.py`):
    ```python
    if notification_type == NotificationTypes.EMAIL_NOTIFICATION:
            # handle email notification
    ```
- Test notifications in Agents Playground: Mock an Activity > Custom activity, paste notification JSON, update conversation ID.
...
- Start Agents Playground:
    ...
    agentsplayground
    ```
- Test chat, tool invocations, and notifications.

## 8. Troubleshooting

- Check logs for errors.
- Refresh tokens as needed (`a365 develop get-token`).
- For port issues, ensure no conflicts.

---

**References:**
- [Quickstart: Python Agent Framework](https://learn.microsoft.com/en-us/microsoft-agent-365/developer/quickstart-python-agent-framework)
- [Testing Guide](https://learn.microsoft.com/en-us/microsoft-agent-365/developer/testing?tabs=python)
- [Observability](https://learn.microsoft.com/en-us/microsoft-agent-365/developer/observability)
- [Notifications](https://learn.microsoft.com/en-us/microsoft-agent-365/developer/notification)
- [YouTube Video Walkthrough](https://www.youtube.com/watch?v=LOtAElJYlmc&list=PLHSY1GJqphddaF5e5h5KJedPUcPhoNHdP)
...
def _create_chat_client(self):
    ...
    if not deployment:
        raise ValueError("AZURE_FOUNDRY_DEPLOYMENT environment variable is required")
    if not api_version:
        raise ValueError("AZURE_FOUNDRY_API_VERSION environment variable is required")
...
- `AZURE_FOUNDRY_ENDPOINT`: Your Azure Foundry service URL
- `AZURE_FOUNDRY_DEPLOYMENT`: Which AI model deployment to use
- `AZURE_FOUNDRY_API_VERSION`: API version for compatibility
